...
whole ESPN account, so Edge treats them as borrowed, not owned: they arrive on the request
...
class EspnAuth:
    ...
    Deliberately not persisted anywhere in Edge. These two values are enough to read that
...
def _get(url: str, params: list[tuple[str, str]] | None = None, headers: dict | None = None,
    ...
    if r.status_code in (401, 403):
        ...
        raise EspnPrivateLeague(
            ...
            "from a browser signed in to ESPN. Edge uses them for this request only and never stores "

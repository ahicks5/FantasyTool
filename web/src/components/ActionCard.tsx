...
const TYPE_ICON: Record<Action["type"], string> = { start: "▲", waiver: "＋", trade: "⇄", hold: "✓" };
  ...
  return (
    <article className={`card relative min-w-0 overflow-hidden p-4 rise rise-${Math.min(delay, 5)} ${a.locked ? "border-dashed" : ""}`}>
      <div className="flex items-center justify-between">
        ...
        {a.confidence ? <ConfidencePill value={a.confidence} /> : a.type === "hold" ? null : <Eyebrow>Priority #{a.priority}</Eyebrow>}
      </div>
      ...
      <div className="mt-3 flex items-start gap-3">
        {a.locked ? (
          ...
          <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-start-soft text-2xl text-start" aria-hidden>
            {TYPE_ICON.hold}
          </span>
        ) : primary ? (

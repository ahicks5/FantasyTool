...
/* ------------------------------------------------------------- the ride ---
   ...
   lamp comes on, and the doors open onto the call sheet.
...
/** The doors open onto the call sheet and the car fades out. */
export const OPEN_MS = 700;

export type RidePhase = "boarding" | "closing" | "sealed" | "rising" | "arrived" | "opening" | "done";
...
export const RIDE_TOTAL_MS = OPEN_AT + OPEN_MS;
 ...
 * clock jumps to the doors opening, so a skip is the same reveal, sooner, rather than
 * the overlay vanishing mid-frame. A tap once the doors are already opening changes
 * nothing.
 */
export function rideState(elapsed: number, skippedAt: number | null = null): RideState {
  const t = skippedAt !== null && elapsed >= skippedAt && skippedAt < OPEN_AT ? OPEN_AT + (elapsed - skippedAt) : elapsed;
  ...
  if (t < OPEN_AT) return { phase: "arrived", floor: top, lines: all };
  if (t < RIDE_TOTAL_MS) return { phase: "opening", floor: top, lines: all };
  return { phase: "done", floor: top, lines: all };
}

/** True once the state can no longer be skipped forward. */
export function pastSkipping(s: RideState): boolean {
  return s.phase === "opening" || s.phase === "done";
}

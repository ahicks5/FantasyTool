...
export function describeError(error: unknown, options: { online?: boolean } = {}): ErrorCopy {
  ...
  if (options.online === false) {
    return {
      ...
      detail: "Edge needs a connection to read your league. This will retry when you are back.",
      canRetry: true,
    };
  }
  ...
  if (status === 429) {
    return {
      ...
      detail: "Edge is asking your league platform to slow down. Give it a minute and try again.",
      canRetry: true,
    };
  }
  if (status === 401) {
  ...
  if (status >= 500) {
    return {
      title: "Edge is having a problem",
